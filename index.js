...
const wordableAPI = {
  ...
  async call({ url = '/words/?random=true', buffer = 50 } = {}) {
    ...
    this.updateLimits({ buffer, response });
  ...
  async updateLimits({ buffer = 50, response } = {}) {
    try {
      const { headers } = response || await this.call({ buffer });
...
// Set an interval to check the limit again in 30 minutes (48 wasted calls per day).
...
const micro = async (req, res) => {
  ...
  try {
    ...
    const { json, status } = await wordableAPI.call({ url: req.url, buffer: 250 });
